...
class UpdateMember(private  val name: String, val users: List<User>, val room: Room, val callback:(Room) -> Unit): API(){
    private val api = retrofit.create(RoomsAPI::class.java)
    private val firebase_cli = FirebaseUtil()
    ...
    private suspend fun updateMember(){
        try {
            val token = firebase_cli.getIdToken()
            ...
            if(token != null) {
                updateAsyncMember(name, userIds, room.id, token).let{
                    callback(it)
                }
            }
        } catch (t: HttpException) {
    ...
    override fun start() {
        ...
        if(user != null) {
            firebase_cli.startWithGettingToken(user) {
                launch(this.job + UI) { updateMember() }
            }
        }
    }
}